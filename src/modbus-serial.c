...
int _modbus_serial_connect(modbus_t *ctx)
{
    ...
    if (ctx->debug)
    {
        printf("Opening %s at %d bauds (%c, %d, %d)\n", ctx_serial->device,
                ctx_serial->baud, ctx_serial->parity, ctx_serial->data_bit,
                ctx_serial->stop_bit);
    }
...
int modbus_serial_set_rts(modbus_t *ctx, int mode)
{
    ...
    if (ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_SERIAL)
    {
        ...
        if (mode == MODBUS_SERIAL_RTS_NONE || mode == MODBUS_SERIAL_RTS_UP
...
int _modbus_serial_select(modbus_t *ctx, fd_set *rset, struct timeval *tv,
        int length_to_read)
{
    int s_rc;
#if defined(_WIN32)
        ...
        int baud, char parity, int data_bit, int stop_bit)
{
    ...
    ctx_serial->onebyte_time = 1000000
            * (1 + data_bit + (parity == 'N' ? 0 : 1) + stop_bit) / baud;
